(ns app.components.update-patient-modal
    (:require
...
(def default-patient-form-data {:id nil
                                :full_name nil
                                :gender nil
                                :dob nil
                                :address ""
...
(defn handleShow
  [patient]
  (reset! patient-form-data patient)
...
(defn handleFormSubmit
  ...
  (if (is-patient-form-valid)
    (pc/update-patient
      (:id @patient-form-data)
      (dissoc @patient-form-data :id)
      (fn [r] (successHandler) (handleClose)))
  ...
  [:> b/form  {:id "form-add-patient" :noValidate true :validated @validated :onSubmit #(handleFormSubmit % successHandler)}  
    [:> b/form.Group
      ...
      [:> b/form.Control {:type "text"
                          ...
                          :value (:full_name @patient-form-data)
                          :required true}]
      ...
      [:div
        ...
        [:> b/form.Group
          [:> b/form.Check {:on-change #(swap! patient-form-data assoc :gender (if (= (-> % .-target .-value) "on") "male"))
                            ...
                            :custom true
                            :inline true
                            :checked (if (= (:gender @patient-form-data) "male") true false)
                            :required true}]
          [:> b/form.Check {:on-change #(swap! patient-form-data assoc :gender (if (= (-> % .-target .-value) "on") "female"))
                            ...
                            :custom true
                            :inline true
                            :checked (if (= (:gender @patient-form-data) "female") true false)
                            :required true}]
    ...
    [:> b/form.Group
      ...
      [im/InputMask {:class "form-control"
                  ...
                  :maskChar ""
                  :pattern "\\d{16}"
                  :value (:oms_number @patient-form-data)
                  :required true
